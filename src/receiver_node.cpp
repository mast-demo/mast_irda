/***
	The looks for IrDA devices and if it finds one
...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
      ...
      gettimeofday(&t, NULL);
      char filename[256];
      sprintf(filename,"data_%ld.bag",t.tv_sec);
      fp = fopen(filename, "w");
			if(stream == NULL) {
				perror("fdopen");
				return -1;
			...
			int received = 0;
			do {
				int n = fread(buf, 1, BUF_SIZE, stream); 
				if(n == 0) break;  // end of file or error
